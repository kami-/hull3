    ...
    if (alive player) then {
        ...
        [] spawn {
            for "_i" from 1 to 60 do {
                TRACE("hull.acre.jip",FMT_2("Waiting for ACRE to initialize TS ID '%1' and spectator list '%2'.",acre_sys_core_ts3id,ACRE_SPECTATORS_LIST));
                sleep 5;
        ...
        DEBUG("hull.acre.jip","Player is dead, setting ACRE spectator to true.");
...
hull3_acre_fnc_setChannels = {
    ...
    _calculatedChannels = [_channelCount, _baseFreq, _channelStep, _sideStep] call hull3_acre_fnc_getCalculatedChannels;
    {
        ...
        TRACE("hull.acre.radio",FMT_2("Set default channels to '%1' for radio '%2'.",_calculatedChannels,_x));
    } foreach _radios;
    DEBUG("hull.acre.radio",FMT_2("Set channels for radios '%1' using default radion '%2' with base frequency '%3' and channel step '%4'.",_radios,_defaultRadio,_baseFreq,_channelStep));
};
