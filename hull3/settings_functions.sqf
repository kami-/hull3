...
hull3_settings_fnc_setPlayerSettings = {
    ...
    DEBUG("hull.settings",FMT_2("Added '%1' rating to player. New rating is '%2'.",AS_ARRAY_2("General", "addRating") call hull3_config_fnc_getNumber,rating player));
    player setVariable ["BIS_noCoreConversations", ["General", "BIS_noCoreConversations"] call hull3_config_fnc_getBool];
    DEBUG("hull.settings",FMT_1("Player variable 'BIS_noCoreConversations' is set to '%1'.",AS_ARRAY_2("General", "BIS_noCoreConversations") call hull3_config_fnc_getBool));
};
...
hull3_settings_fnc_setModuleVariables = {
    if (isNil {hull3_marker_isGroupEnabled}) then {
        ...
        DEBUG("hull.settings.marker",FMT_1("Group marker module was not found. Setting Group Marker enabled from config to '%1'.",AS_ARRAY_2("Marker", "isGroupEnabled") call hull3_config_fnc_getBool));
    };
    if (isNil {hull3_marker_isFireTeamEnabled}) then {
        ...
        DEBUG("hull.settings.marker",FMT_1("Fire Team Member marker module was not found. Setting Fire Team Member marker enabled from config to '%1'.",AS_ARRAY_2("Marker", "isFireTeamEnabled") call hull3_config_fnc_getBool));
    };
};
